...
original_author="Drsims"
original_name="flask__project"
original_urlname="Flask-_Project"
original_description="Awesome flask__project created by Drsims" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask__project "${name}"
...
then
    ...
    echo "then run 'flask__project' or 'python -m flask__project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask__project' or 'python -m flask__project' or 'poetry run flask__project'"
fi
