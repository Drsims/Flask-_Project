...
- 🎯 Entry points to execute your program using `python -m <flask__project>` or `$ flask__project` with basic CLI argument parsing.
...
---
# flask__project Flask Application

Awesome flask__project created by Drsims
...
```bash
git clone https://github.com/Drsims/Flask-_Project flask__project
cd flask__project
make install
```
...
```bash
flask__project create-db   # run once
flask__project populate-db  # run once (optional)
flask__project add-user -u admin -p 1234  # ads a user
flask__project run
```
